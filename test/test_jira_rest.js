...
function handleApprovalAction (issueKey, decision, comment) {
  const options = {
    method: 'POST',
...
function createCustomerRequest (summary, description) {
  const options = {
    method: 'POST',
  ...
  describe('Discovery', () => {
    it('should return correct meta data', async () => {
      ...
      expect(resp.body).to.eql(
        {
          image: {
            href: 'https://test-host:3001/dev/images/connector.png'
          },
          object_types: {
            card: {
              pollable: true,
