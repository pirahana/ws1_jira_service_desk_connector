...
/**
 * Validate the JWT in the request.  If valid, call next(), if not, show an error response
 * @param  {} req
 * @param  {} res
 * @param  {} next
 */
function vidmValidate(req, res, next) {
  ...
  if (authorization) {
    ...
    verifyAuth(authorization, vidmOptions).then(function (decoded) {
      // Everything was correct and valid
      res.locals.jwt = decoded
      res.locals.vidm = authKeyURL
      next()
    }).catch(function (err) {
      console.log('Identity verification failed:', err)
      res.status(401).json({
        message: `Identity verification failed! ${err}`
      })
    })
  } else {
    res.status(401).send({
 ...
 * @param  {} authorization contents of the Authorization header, e.g. 'Bearer sometoken'
...
/**
 * Given a JWT, return the path to the vIDM public key of the issuer
 * @param  {} authorization
 */
function audClaimPublicKeyURL(authorization) {
  const auth = authorization.replace('Bearer ', '').trim()
  ...
  url.pathname = '/SAAS/API/1.0/REST/auth/token'
  url.search = 'attribute=publicKey&format=pem'
  return url
}
/**
...
function getPublicKey(options) {
  if (pubKeyCache && (options.vIdmPubKeyUrl in pubKeyCache) && pubKeyCache[options.vIdmPubKeyUrl].expiresAtTime > Date.now()) {
    return Promise.resolve(pubKeyCache[options.vIdmPubKeyUrl].contents)
  }
  ...
  return rp(options.vIdmPubKeyUrl).then(function (data) {
    ...
    return Promise.reject(new Error(`Failed to retrieve public key: ${response.statusCode}`))
  })
}

exports.validate = vidmValidate